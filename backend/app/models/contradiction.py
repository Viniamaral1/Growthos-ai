...
class ContradictionRecord(Base):
    ...
    __table_args__ = (UniqueConstraint('company_id','signature',name='uq_contradiction_company_signature'),)
    id: Mapped[int] = mapped_column(primary_key=True,index=True)
    company_id: Mapped[int] = mapped_column(ForeignKey('companies.id',ondelete='CASCADE'),nullable=False,index=True)
    space_id: Mapped[int|None] = mapped_column(ForeignKey('knowledge_spaces.id',ondelete='SET NULL'),nullable=True,index=True)
    ...
    contradiction_type: Mapped[str] = mapped_column(String(60),nullable=False,index=True)
